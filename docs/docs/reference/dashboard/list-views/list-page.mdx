...
<GenerationInfo sourceFile="packages/dashboard/src/lib/framework/page/list-page.tsx" sourceLine="552" packageName="@vendure/dashboard" since="3.3.0" />
...
<MemberInfo kind="property" type={`InlineDropdownItem[]`}   />

Optional dropdown menu items to display in the action bar's overflow (⋮) menu.
These are forwarded to the [PageActionBar](/reference/dashboard/page-layout/page-action-bar#pageactionbar).
### facetedFilters
...
Gate it with a [PermissionGuard](/reference/dashboard/components/permission-guard#permissionguard) to mirror the create button's permissions.
