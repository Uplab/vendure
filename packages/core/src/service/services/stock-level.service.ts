...
export class StockLevelService {
    ...
    async getStockLevelsForVariant(ctx: RequestContext, productVariantId: ID): Promise<StockLevel[]> {
        return this.getStockLevelsForVariantLoader(ctx).load(productVariantId);
    }
    ...
    /**
     * The Admin API `ProductVariant.stockLevels` field is resolved once per variant, so listing
     * variants costs one channel-filtered query per row. The loader is held on the RequestContext
     * and the channel filter below is read from that same ctx, so one loader per ctx cannot mix
     * channels. `cache: false` for the same reason as {@link getStockLevelLoader}: batch, but do
     * not memoize, so a write earlier in the request is not masked.
     */
    private getStockLevelsForVariantLoader(ctx: RequestContext): DataLoader<ID, StockLevel[]> {
        return this.requestCache.get(
            ctx,
            'StockLevelService.stockLevelsForVariant',
            () =>
                new DataLoader<ID, StockLevel[]>(
                    ids => this.batchLoadStockLevelsForVariant(ctx, ids as ID[]),
                    { cache: false },
                ),
        );
    }

    private async batchLoadStockLevelsForVariant(ctx: RequestContext, ids: ID[]): Promise<StockLevel[][]> {
