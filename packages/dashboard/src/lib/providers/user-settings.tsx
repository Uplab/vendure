 ...
 * how table settings are written means existing saved settings need migrating on load, and
 * add a correspondingly-gated step to {@link migrateTableSettings}. Bumping never re-runs
 * the steps that came before: a stored copy only ever runs the steps above its own stamp.
 */
export const TABLE_SETTINGS_VERSION = 1;

/**
 * Version 1: drops saved `columnFilters` entries that are empty. See
 * {@link migrateTableSettings} for why.
 */
function dropEmptyColumnFilters(settings: UserSettings): UserSettings {
 ...
 *
 * Each version is a separately gated step, run only against settings stamped below it, so a
 * later bump does not re-run version 1's drop over an empty `columnFilters` that by then is
 * a genuine user clear. Settings stamped at or above the current version — including a copy
 * written by a newer dashboard the install has since rolled back from — are returned
 * unchanged, by reference, so callers can tell whether anything needs writing back.
 */
export function migrateTableSettings(settings: UserSettings): UserSettings {
    const fromVersion = settings.tableSettingsVersion ?? 0;
    if (fromVersion >= TABLE_SETTINGS_VERSION) {
        return settings;
    }
    let migrated = settings;
    if (fromVersion < 1) {
        migrated = dropEmptyColumnFilters(migrated);
    }
    return { ...migrated, tableSettingsVersion: TABLE_SETTINGS_VERSION };
}
