...
describe('migrateTableSettings', () => {
    ...
    it('migrates settings stamped below the current version', () => {
        // Version 0 is the un-stamped shape, whether the stamp is absent or explicitly 0.
        const migrated = migrateTableSettings({
            tableSettings: { 'product-list': { columnFilters: [], pageSize: 25 } },
            tableSettingsVersion: 0,
        } as UserSettings);
    ...
    it('returns settings stamped above the current version untouched', () => {
        // A copy written by a newer dashboard that this install has rolled back from. Running
        // the migration would drop filters the newer version treats as a real clear, and stamp
        // the version backwards; leaving it alone lets the newer version pick up where it left off.
        const settings = {
            tableSettings: { 'product-list': { columnFilters: [] } },
            tableSettingsVersion: TABLE_SETTINGS_VERSION + 1,
        } as UserSettings;

        const migrated = migrateTableSettings(settings);

        expect(migrated).toBe(settings);
        expect(migrated.tableSettingsVersion).toBe(TABLE_SETTINGS_VERSION + 1);
