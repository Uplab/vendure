...
/**
 * Reads the saved column filters for a page, distinguishing "no value has been saved"
 * from "an empty value has been saved" — `undefined` alone cannot survive the trip out
 * of `page.evaluate`.
 */
...
test.describe('List page column filter persistence', () => {
    // #TBD — `ListPage`'s `defaultColumnFilters` applies a page's default filters only until
    ...
    // persisted whatever it was told.
    ...
    // #TBD — clearing every filter is a deliberate choice and has to be saved as such, so
    // that a page's `defaultColumnFilters` are not re-applied on the next visit.
    test('should save cleared filters as an empty state that survives a reload', async ({ page }) => {
        const lp = productList(page);
        await lp.goto();
        await lp.expectLoaded();
        ...
        // Saved, but empty: the user has configured the filters and chosen to have none.
        const afterClearing = await readSavedColumnFilters(page, PAGE_ID);
        expect(afterClearing.saved).toBe(true);
        expect(afterClearing.value).toEqual([]);
        ...
        expect(afterReload.value).toEqual([]);
    });
});
